...
A 0.3.0 mandava rodar `quarto render quarto --no-freeze` depois de qualquer
mudança de layout — em nove lugares: README, NEWS, `?theoviz`, `?tinta`,
comentários de `R/paleta.R` e a vignette.

**Essa opção não existe no Quarto 1.9.** Ela é repassada ao pandoc, que responde
`Unknown option --no-freeze`, e o render do projeto inteiro falha. Descoberto em
2026-08-15, ao migrar de fato os três projetos para o modo escuro — que foi a
primeira vez que alguém executou a instrução.
...
`build:quarto` não é enfeite sobre `quarto render`: ele encadeia a poda das
fontes do ggiraph — 326 MB numa execução medida — e o sync para o Astro. Um
`quarto render` cru deixa os dois de fora.

## A migração pendente na 0.3.0 foi feita

Os três itens que a 0.3.0 listou como "não é meu para fazer" foram executados
nos repos dos projetos, em 2026-08-15: `cargos-executivo-federal`,
`relatorios-slu` e `anuario-mineral-brasileiro` passaram a pedir
`modo = "escuro"`, o `TINTA_2 <- "#8a8a85"` do SLU foi removido, e os 14
artigos que esta máquina consegue executar foram re-renderizados.
...
apagar `quarto/_freeze/` e rodar `npm run build:quarto`.
...
4. **Apagar `quarto/_freeze/` e rodar `npm run build:quarto`** no site depois de cada migração.
...
3. **Apagar `quarto/_freeze/` e rodar `npm run build:quarto`** no site depois de subir a versão,
